...
const Order = ({order_id}) => {
  ...
  useEffect(() => {
    ...
    const handleSelectChange = (category, event) => {
        ...
        const newOrder = { category, selectedItem, quantity: 1, amount };
        
        // orders状態に新しい注文を追加する
        setOrders(prevOrders => [...prevOrders, newOrder]);
    };
    
    // カテゴリーとアイテム名に基づいて注文の数量を変更する関数を修正する
    const handleQuantityChange = (category, selectedItem, event) => {
        const quantity = parseInt(event.target.value);
        setOrders(prevOrders =>
            prevOrders.map(order =>
                order.category === category && order.selectedItem === selectedItem
                    ? { ...order, quantity }
                    : order
            )
        );
            ...
            {menu && (
                ...
                <form onSubmit={handleSubmit} className=" ">
                    ...
                    {Object.keys(menu).map(category => (
                      ...
                      {orders
    .filter(order => order.selectedItem && order.category === category)
    .map(order => (
        <Grid item xs={4} className="flex justify-end" key={order.selectedItem}>
            <FormControl className="w-36">
                <Select
